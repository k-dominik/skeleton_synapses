...
def get_and_report_env(name, default, constructor=str):
    """
    ...
    val = os.getenv(name)
    if val is None:
        logger.debug('{} not set, using default.'.format(name, default))
        val = default
    logger.info('{} value is {}'.format(name, val))
    return constructor(val)


THREADS = get_and_report_env('SYNAPSE_DETECTION_THREADS', 3, int)
NODES_PER_PROCESS = get_and_report_env('SYNAPSE_DETECTION_NODES_PER_PROCESS', 500, int)
RAM_MB_PER_PROCESS = get_and_report_env('SYNAPSE_DETECTION_RAM_MB_PER_PROCESS', 5000, int)
